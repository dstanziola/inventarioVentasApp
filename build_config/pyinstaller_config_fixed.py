...
Archivo: build_config/pyinstaller_config_fixed.py
Fecha: 2025-08-02
...
class LogoConverter:
    ...
    def create_ico_from_png(self):
        ...
        print("[LOGO] Convirtiendo logo a formato ICO...")
        
        # Verificar que PIL está disponible
        try:
            from PIL import Image
        except ImportError:
            print("   [SETUP] Instalando Pillow para conversión de imágenes...")
            import subprocess
        ...
        if LOGO_320.exists():
            ...
            print(f"   [OK] Usando logo fuente: {source_logo.name}")
        elif LOGO_TRANSP.exists():
            source_logo = LOGO_TRANSP
            print(f"   [OK] Usando logo transparente: {source_logo.name}")
        elif LOGO_2000.exists():
            source_logo = LOGO_2000
            print(f"   [OK] Usando logo alta resolución: {source_logo.name}")
        else:
            print("   [ERROR] No se encontró ningún archivo de logo")
            return None
        ...
        try:
            ...
            print(f"   [OK] Archivo ICO creado: {ICON_PATH.name}")
            return ICON_PATH
            
        except Exception as e:
            print(f"   [ERROR] Error creando ICO: {e}")
            return None
...
'''
    ...
    print(f"[OK] Archivo .spec creado: {spec_file_path.name}")
...
'''
    ...
    print(f"[OK] Archivo version_info.txt creado: {version_file_path.name}")
...
'''
    ...
    build_script_path = PROJECT_ROOT / "build_portable_fixed.bat"
    with open(build_script_path, 'w', encoding='utf-8') as f:
        f.write(build_script_content)
    
    print(f"[OK] Script de construcción creado: {build_script_path.name}")
    return build_script_path

if __name__ == "__main__":
    print(f"[INIT] Configurando construcción para {APP_NAME} v{VERSION}")
    print(f"[INIT] Directorio del proyecto: {PROJECT_ROOT}")
    ...
    print("\n[COMPLETE] Configuración completada. Archivos generados:")
    print(f"   - {spec_file.name}")
    print(f"   - {version_file.name}")
    print(f"   - {build_script.name}")
    if ico_created:
        print(f"   - {ico_created.name}")
    print("\n[NEXT] Para construir el ejecutable, ejecuta: build_portable_fixed.bat")
